...
const email = (seller, product, buyer) => `
  <img alt="OnlineStore" src=".github/logo.png" width="300px" />
  <hr>
  ...
  <br><br>
  <p>Atenciosamente, Equipe OnlineStore</p>
`;
    ...
    try {
      ...
      const createOrdersPromise = filteredItems.map(async (item) => {
        ...
        await mailer.sendMail({
          to: seller.email,
          from: 'no-reply@onlinestore.com.br',
  ...
  async update(req, res) {
    try {
      ...
      if (!acceptedActions.includes(action))
        return res.send("Can't do this actions");
